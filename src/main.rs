...
impl Zone_3D{
    ...
    fn subtract(mut self,x:u64,y:u64,z:u64)->Zone_3D{
        self.capacity+=1;
        self.segments = self.segments.into_iter().filter_map(|mut seg| {
...
const NO_OF_HOSTS_PER_SEGMENT:[u8;2] = [2,3];
//Disease 
...
const DEFECATION_RATE:f64 = 25.0; //Number times a day host is expected to defecate
...
//Resolution
const STEP:[[usize;3];2] = [[20,20,1],[30,30,30]];  //Unit distance between hosts ->Could be used to make homogeneous zoning (Might not be very flexible a modelling decision)
...
impl host{
        ...
        for zone in (0..space.len()).rev(){
            ...
            if &space[zone].capacity>&0 && zone>0{ //If succeeding zones (obviously zone 0 doesn't have do to this - that needs to be done with a replace mechanism)
                ...
                vector.iter_mut().for_each(|mut x| {
                    if x.zone == zone-1 && x.time>ages[zone-1] && __>0 && x.motile == 0{ //Hosts in previous zone that have spent enough time spent in previous zone
                        //Find the first available segment
                        __ -= 1;
                        space[zone-1] = space[zone-1].clone().subtract(x.origin_x.clone(),x.origin_y,x.origin_z.clone()); //move host from previous zone
                        ...
                        [x.origin_x,x.origin_y,x.origin_z,__,___,____] = space[zone].add();                            
                    }
                })
    ...
    fn deposit(self, consumable: bool)->host{ //Direct way to lay deposit from host. The function is 100% deterministic and layering a probability clause before this is typically expected
        ...
        else{host{infected:inf,motile:2,zone:zone,prob1:prob1,prob2:prob2,x:x,y:y,z:z,age:0.0,time:0.0,origin_x:x as u64,origin_y:y as u64,origin_z:z as u64,restrict:restriction,range_x:range_x,range_y:range_y,range_z:range_z}}
    }
    ...
    fn generate_in_grid(zone:&mut Zone_3D,hosts:&mut Vec<host>){  //Fill up each segment completely to full capacity in a zone with chickens. Also update the capacity to reflect that there is no more space
        ...
        zone.segments.iter_mut().for_each(|mut x| {
            for _ in 0..x.capacity.clone() as usize{hosts.push(host::new(zone_no,0.2,x.origin_x as f64,x.origin_y as f64,x.origin_z as f64,RESTRICTION,x.range_x,x.range_y,x.range_z));}
            x.capacity = 0;
            zone.capacity = 0;
        });
    }
...
fn main(){
    ...
    wtr.flush().unwrap();
    println!("{} {} {} {} {}",GRIDSIZE[0][0],GRIDSIZE[0][1],LENGTH,GRIDSIZE.len(), 0); //Last 5 lines are going to be zone config lines that need to be picked out in plotter.py
